...
def plot_network(network_path,ds,datasets, titles=['(a)','b)','(c)','(d)']):
    ...
    for i in range(4):
        ...
        for bus in network.buses.iterrows():
            ...
            filter = [x and y for x,y in zip(network.generators.bus==bus[0],network.generators.carrier=='wind')]
            wind = sum(network.generators[filter].p_nom_opt)
            filter = [x and y for x,y in zip(network.generators.bus==bus[0],network.generators.carrier=='solar')]
            solar = sum(network.generators[filter].p_nom_opt)
            filter = [x and y for x,y in zip(network.generators.bus==bus[0],network.generators.carrier=='OCGT')]
